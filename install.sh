...
REPO_NAME="${YNPM_PACKAGE:-ychrome}"
BIN_NAME="${YNPM_BIN:-ychrome}"
SCOPE="@ygghq"
...
case "$(uname -s)/$(uname -m)" in
  ...
  *) echo "install.sh: no prebuilt ychrome binary for $(uname -s)/$(uname -m) (ychrome is a WebKitGTK app: linux x64 and arm64 ship today)" >&2; exit 1 ;;
esac
