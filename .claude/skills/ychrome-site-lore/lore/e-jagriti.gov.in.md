...
The account password IS in vault item `e-jagriti.gov.in` but only **one host's** agent has
...
Inject the password with `web fill-vault --item e-jagriti.gov.in --field password
