...
## edaakhil.nic.in is DEFUNCT -> use e-consumerforum.example.test
...
## Creds
e-consumerforum account for this owner exists (registered Consumer). Reference the vault by domain
e-consumerforum.example.test. (Vault WRITE returned server 401 during this run; account creds were saved
...
persisted -- vault item `e-consumerforum.example.test` (yggterm ROUND 16's vault-write fix filed the
