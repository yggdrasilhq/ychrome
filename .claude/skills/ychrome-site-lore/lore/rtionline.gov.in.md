...
2. **`clients: 0` on dev means the daemon there is `yggterm-headless`**, which registers no GUI
   client — not that no rail exists. **jojo had a live GUI client AND an unlocked vault the whole
...
```
jojo: terminal new --no-activate → sleep 3 → send `ychrome --profile agent-<n> <payment-url>`
