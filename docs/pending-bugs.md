...
is present on dev, jojo and oc with the same mtime, and the whole OpenWrt
...
fast of NTP time`, jojo/dev/oc agree on the epoch to the second, and drift
...
no `--delete`, so any future clock skew between jojo and manin silently resolves
