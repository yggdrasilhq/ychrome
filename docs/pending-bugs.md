...
## ⚠ THE JOURNAL STILL CANNOT ANSWER FOR DAEMON LIFETIME ON ITS OWN

**Status:** OPEN, and small. Left over from the YouTube-segfault entry, which is fixed and
deleted (see `git log` for `substrate: the engine's own compositing flag was the crash`).

Every daemon transition in `journal.jsonl` is a `daemon_stop` / `daemon_start` **pair**, so a
crash appears as a `daemon_start` **alone** and the journal cannot distinguish a crash from a
restart. `daemon_exit` exists but is written by the reaping thread in whichever process spawned
the daemon — and a CLI invocation exits long before the daemon dies, so in practice it is
almost never there (measured: 0 such lines across several real crashes).

⇒ **What is wanted:** a `daemon_exit` written by something that outlives the daemon, so the
journal answers for lifetime directly rather than by the absence of a `daemon_stop`.

⚠ The kept `daemon.stderr.log` (0600, rolled past 4 MB) is the channel that does not depend on
who is still watching, and it is what made the segfault diagnosable at all. This entry is about
the journal being self-sufficient, not about losing the diagnostics again.

**Falsifier:** kill -SEGV a daemon from outside; `journal.jsonl` names the signal.
