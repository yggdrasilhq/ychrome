...
India Post booking by card on jojo: *"ychrome-vault edit/or see details of card is broken and no
...
operator's machine (`agent-engine.md` §4: "how agent browsing finally stops touching jojo"), and
the co-browse doctrine prefers dev for exactly that reason. But `server app web fill-card` needs a
registered GUI client — dev has **0**, jojo has **1** — so **every card payment is forced onto the
...
suspicion: `ychrome-vault card` works on dev, and dev and jojo run a **byte-identical** yggterm
...
cart — then handed to a second agent on jojo at 05:00 to pay **Rs 23**.
...
went through **with the unlock preserved** (verified live on jojo: 1120 items,
