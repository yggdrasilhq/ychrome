...
anything that renders (`data-fabric`: *"Prefer dev over guihost… guihost is the laptop
...
below. Any flow that needs a second factor has to source the code from guihost even
...
Epoch compared across the fleet: **guihost `…997` (correct); dev `…925`; oc `…925`.**
dev and oc are LXCs on **manin** and share its `CLOCK_REALTIME`, so this is one
clock, wrong, serving two of the three hosts. guihost is a separate machine and is
fine.
...
`--delete`. With a 72 s offset between guihost and manin, two edits to the same
memory inside a 72 s window resolve by the wrong winner — a file genuinely newer
on guihost can lose to a stale manin copy.
...
**Found on guihost, 2026-07-31, by the operator, who watched an IBKR login window
...
**Deploy state at time of writing:** guihost has the new binary with a **stale
