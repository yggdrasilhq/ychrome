...
(the version on dev and on jojo, checked with `pkg-config --modversion
...
jojo predated the script's own metadata block, so it parsed to the documented
...
ruleset had to be copied by hand; it was on jojo and had never been on dev,
...
`$HOME`, including jojo's exact header-less `youtube-adblock` and a hand-copied
...
**Not proven.** Nothing has been exercised in the yggterm GUI on jojo: no
